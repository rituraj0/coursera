...
Application::Application(char *infile) {
	 ...
	 * Init all nodes
	 */
	cout<<"Starting with "<<par->EN_GPSZ<<" nodes"<<endl;

	for( i = 0; i < par->EN_GPSZ; i++ ) {

		cout<<"Init at node "<<i<<endl;
		...
		cout<<"Calling joinaddr "<<joinaddr.getAddress()<<endl;
		...
		cout<<"New address is "<<(addressOfMemberNode->getAddress())<<endl;
